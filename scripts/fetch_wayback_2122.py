#!/usr/bin/env python3
"""
fetch_wayback_2122.py — Scrapes futbolaspalmas.com 2021-2022 season data
from Wayback Machine archives.

For 2021-2022, the site uses the same structure as 2022-2023:
  - Benjamin (7 groups):        futbolaspalmas.com/1benjaminN/
    Subpage: /1benjaminN/primera-benjamin-grupo-N-benjamin-primeraN.html
    (group 4 only available as main page)
  - Benjamin Preferente (2):    futbolaspalmas.com/1benjamin-prefe1/ and /1benjamin-prefe2/
    Subpage: /1benjamin-prefeN/benjamin-preferente-grupo-N-benjaminprefeN.html
  - Prebenjamin (3 groups):     futbolaspalmas.com/1prebenjaminN/
    Subpage: /1prebenjaminN/primera-benjamin-grupo-N-prebenjamin-primeraN.html

Best snapshots are from June 2022 (end of season, most complete data).
Some groups only have mid-season data (April or earlier).

Output: scripts/wayback_2122_raw.json
"""
...
OUTPUT_PATH = os.path.join(SCRIPT_DIR, "wayback_2122_raw.json")

CDX_URL   = "https://web.archive.org/cdx/search/cdx"
WAYBACK   = "https://web.archive.org/web"
SITE      = "futbolaspalmas.com"

# End-of-season snapshots (June 2022)
FROM_DATE = "20220601"
TO_DATE   = "20220630"
# Mid-season fallback (January-April 2022)
FALLBACK_FROM = "20220101"
FALLBACK_TO   = "20220430"
# Earlier fallback (Oct-Dec 2021)
EARLY_FROM = "20211001"
EARLY_TO   = "20211231"
...
# ─── KNOWN GROUPS ─────────────────────────────────────────────────────────────
# Based on CDX discovery. 2021-2022 had 7 benjamin groups, 2 prefe, 3 prebenjamin.
# Group 4 benjamin only archived as main page (no subpage snapshot found).

KNOWN_GROUPS = [
    # ── Benjamin Primera (1-7) ─────────────────────────────────────────────
    {
        "slug": "1benjamin1",
        "category": "benjamin",
    ...
    },
    {
        "slug": "1benjamin7",
        "category": "benjamin",
        "main_url": "https://futbolaspalmas.com/1benjamin7/",
        "subpage": "https://futbolaspalmas.com/1benjamin7/primera-benjamin-grupo-7-benjamin-primera7.html",
    },
    ...
    },
    {
        "slug": "1prebenjamin3",
        "category": "prebenjamin",
        "main_url": "https://futbolaspalmas.com/1prebenjamin3/",
        "subpage": "https://futbolaspalmas.com/1prebenjamin3/primera-benjamin-grupo-3-prebenjamin-primera3.html",
    },
]
...
def discover_snapshots():
    ...
    Preference order:
      1. subpage in June 2022 (end of season, most complete)
      2. subpage in Jan-Apr 2022 (mid season)
      3. subpage in Oct-Dec 2021 (early season)
      4. main page in June 2022
      5. main page in Jan-Apr 2022
      6. main page in Oct-Dec 2021
    """
    groups_out = []
    for g in KNOWN_GROUPS:
        ...
        # 1. Subpage in June 2022
        if g.get("subpage"):
            ts, snap_url = cdx_best_snapshot(g["subpage"], FROM_DATE, TO_DATE)
            if ts:
                print(f"  Found subpage in June 2022: {ts}")
            else:
                # 2. Subpage in Jan-Apr 2022
                time.sleep(0.3)
                ts, snap_url = cdx_best_snapshot(g["subpage"], FALLBACK_FROM, FALLBACK_TO)
                if ts:
                    print(f"  Found subpage in Jan-Apr 2022: {ts}")
                else:
                    # 3. Subpage in Oct-Dec 2021
                    time.sleep(0.3)
                    ts, snap_url = cdx_best_snapshot(g["subpage"], EARLY_FROM, EARLY_TO)
                    if ts:
                        print(f"  Found subpage in Oct-Dec 2021: {ts}")

        # 4-6. Fall back to main page
        if not ts:
            time.sleep(0.3)
            ts, snap_url = cdx_best_snapshot(g["main_url"], FROM_DATE, TO_DATE)
            if ts:
                print(f"  Found main page in June 2022: {ts}")
            else:
                time.sleep(0.3)
                ts, snap_url = cdx_best_snapshot(g["main_url"], FALLBACK_FROM, FALLBACK_TO)
                if ts:
                    print(f"  Found main page in Jan-Apr 2022: {ts}")
                else:
                    time.sleep(0.3)
                    ts, snap_url = cdx_best_snapshot(g["main_url"], EARLY_FROM, EARLY_TO)
                    if ts:
                        print(f"  Found main page in Oct-Dec 2021: {ts}")
...
def parse_standings(html):
    """
    Parses standings from 2021-2022 group pages.

    The 2021-2022 site uses div-based standings with inline width% styles
    (same format as 2022-2023).
    ...
    Team names are extracted from calendarioClasificacion() onclick handlers.
...
def main():
    print("=== Wayback Machine 2021-2022 Scraper ===\n")
