#!/usr/bin/env python3
"""
import_wayback_2223.py — Imports wayback_2223_raw.json into futbolbase.db.

Only touches season 2022-2023. Does NOT modify other season data.
Maps futbolaspalmas.com slugs to internal group codes.

Group types for 2022-2023:
  1benjaminN     → Benjamin Primera GC  → code: GC1..GC6
  1benjamin-prefeN → Benjamin Preferente → code: BPGC1..BPGC2
  1prebenjaminN  → Prebenjamin Primera  → code: PGC1..PGC2
"""
...
RAW_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "wayback_2223_raw.json")
...
def main():
    if not os.path.exists(RAW_PATH):
        print(f"ERROR: {RAW_PATH} not found. Run fetch_wayback_2223.py first.")
        sys.exit(1)
