...
- **Registro.** Cada pantalla nueva sustituye a `pendiente` en `SCREEN_MAP` (`src/screens.js`) y entra en `STATIC_ASSETS` de `sw.js`: `test_sw_fixes.mjs` compara la lista con el grafo real de `app.js`. El router ya resuelve sus parámetros (`resolveParams`), su padre (`parentOf` → `#/explorar`), su barra (`activeTab`) y su modo de historial (`q` en Explorar y Goleadores es `replace`).
- **Enlaces que ya llegan desde B2:**
  ...
  - Equipo: `#/equipo?s&g&t` desde las filas de la Tabla y «Ver toda la temporada 2025/26» de D; `…#calendario` desde «calendario completo» de la portada. Su calendario puede reutilizar `teamCalendar(team, group, { today, shields })`, exportada por `screen-home.js`.
  ...
  - Copa: `#/copa?s&g` desde las filas de «Verano», el «‹» de un partido de torneo o de cuadro, y la redirección de los grupos que no son de liga (MC\* incluidos).
...
- **El ancla tras la ruta** (`#buscar`, `#calendario`): el router desplaza hasta ella al avanzar, pero lleva el foco al `h1` (§8). Para que Explorar enfoque su buscador, su `mount` lo enfoca, o el router enfoca el ancla cuando es un control.
...
- **Limpieza que B3 puede hacer:** las funciones de `state.js` que se quedaron sin papel (decisión 1); `getCurrentSeason`, si hace falta, es `PORTAL.season` en una línea.
...
- **Carga de datos** (§12.5, decisión 5): `defer`, la carga perezosa de `data-stats.js` y `data-maspalomas-cup-2026.js`, y la baja de `data-matchdetail-keys.js`. Al retirarlo, lo nombran `test_rediseno_index.mjs` (la lista de los 9 inmediatos), `test_js_modules.mjs` y `test_sw_fixes.mjs`, y `fixture-site.mjs` sirve un sustituto que sobra.
...
- **PWA:** el `apple-touch-icon` PNG, el manifiesto y el `preload` (decisión 26).
